...
extension Mongo.ConnectionSettings
{
    ...
    struct Authentication:Equatable, Sendable
    {
        ...
        func mechanism(handshake:ServerHandshake) -> Mechanism?
        {
            if let mechanism:Mechanism = self.mechanism
            {
                return mechanism
            }
            if let supported:[String] = handshake.saslSupportedMechs 
            {
                for supported:String in supported 
                {
                    if  let supported:Mechanism =
                        .init(rawValue: supported)
                    {
                        return supported
                    }
                }
